...
class Agent(common.Module):
    def __init__(self, config, obs_space, act_space, step):
        ...
        if config.separate_wm:
            self.wm = SeparatedWorldModel(config, obs_space, self.tfstep)
        else:
    ...
    def report(self, data):
        report = {}
        data = self.wm.preprocess(data)
        if self.config.separate_wm:
            openl_joint, openl_main, openl_disen, openl_mask = self.wm.video_pred(data)
            report[f'openl_joint'] = openl_joint
            report[f'openl_main'] = openl_main
            report[f'openl_disen'] = openl_disen
            report[f'openl_mask'] = openl_mask
        else:
...
class SeparatedWorldModel(common.Module):
    def __init__(self, config, obs_space, tfstep):
        ...
        self.disen_rssm = common.EnsembleRSSM(**config.rssm)
        self.disen_encoder = common.Encoder(shapes, **config.encoder)
        self.disen_heads = {
            # 'decoder': common.Decoder(shapes, **config.decoder),
            'reward': common.MLP([], **config.reward_head),
        }
        if config.pred_discount:
            self.disen_heads['discount'] = common.MLP([], **config.discount_head)
        # for name in config.grad_heads:
        #     assert name in self.disen_heads, name
        self.disen_model_opt = common.Optimizer('disen', **config.model_opt)
        self.disen_heads_opt = {
            'reward': common.Optimizer('disen_reward', **config.model_opt),
        }
        if config.pred_discount:
            self.disen_heads_opt['discount'] = common.Optimizer('disen_pcont', **config.model_opt)
        # negative signs for reward/discount here

        self.disen_only_image_head = common.Decoder(shapes, **config.decoder)

        ################
        # Joint Decode #
        ################
        self.image_head = common.DecoderMask(shapes, **config.decoder)
        self.disen_image_head = common.DecoderMask(shapes, **config.decoder)
        self.joint_image_head = common.DecoderMaskEnsemble(
            self.image_head, self.disen_image_head
        )
    
    def train(self, data, state=None):
        with tf.GradientTape() as model_tape, tf.GradientTape() as disen_tape:
            model_loss, disen_loss, state, outputs, metrics = self.loss(data, state)
        modules = [self.encoder, self.rssm, self.joint_image_head, *self.heads.values()]
        disen_modules = [self.disen_encoder, self.disen_rssm, self.joint_image_head, self.disen_only_image_head]
        metrics.update(self.model_opt(model_tape, model_loss, modules))
        metrics.update(self.disen_model_opt(disen_tape, disen_loss, disen_modules))
        return state, outputs, metrics

    def loss(self, data, state=None):
        data = self.preprocess(data)
        if self.config.disen_action_type == 'main':
            disen_action = data['action']
        elif self.config.disen_action_type == 'none':
            disen_action = tf.zeros_like(data['action'])
        elif self.config.disen_action_type == 'disen':
            disen_action = data['disen_action']
        else:
        ...
        post_disen, prior_disen = self.disen_rssm.observe(
            embed_disen, disen_action, data['is_first'], state)
        kl_loss_disen, kl_value_disen = self.disen_rssm.kl_loss(post_disen, prior_disen, **self.config.kl)
        feat_disen = self.disen_rssm.get_feat(post_disen)
        # assert len(kl_loss_disen.shape) == 0

        # Optimize disen reward/pcont till optimal
        disen_metrics = dict(reward={}, discount={})
        loss_disen = dict(reward=None, discount=None)
        for _ in range(self.config.num_reward_opt_iters):
            ...
            for name, head in self.disen_heads.items():
                disen_metrics[name] = self.disen_heads_opt[name](
                    disen_gradient_tapes[name], loss_disen[name], [head])


        likes_disen = {}
        losses_disen = {'kl_disen': kl_loss_disen}
        for name, head in self.disen_heads.items():
            grad_head = (name in self.config.grad_heads)
            inp = feat_disen if grad_head else tf.stop_gradient(feat_disen)
            out = head(inp)
            dists = out if isinstance(out, dict) else {name: out}
            for key, dist in dists.items():
                like_disen = tf.cast(dist.log_prob(data[key]), tf.float32)
                likes_disen[key] = -like_disen.mean()
                losses_disen[key] = -like_disen.mean()
        disen_only_image_pred = self.disen_only_image_head(feat_disen)['image']
        disen_only_image_like = tf.cast(tf.reduce_mean(disen_only_image_pred.log_prob(
            data['image'])), tf.float32) * self.config.disen_scales.get('disen_only', 1.0)
        likes_disen['disen_only'] = disen_only_image_like

        # Joint decode
        image_pred_joint, _, _, _ = self.joint_image_head(feat, feat_disen)
        image_like = tf.cast(tf.reduce_mean(image_pred_joint.log_prob(
            data['image'])), tf.float32)
        likes['image'] = image_like
        likes_disen['image'] = image_like


        model_loss = kl_loss.mean() - sum(
            self.config.loss_scales.get(k, 1.0) * v for k, v in likes.items())
        outs = dict(
            embed=embed, feat=feat, post=post,
            prior=prior, likes=likes, kl=kl_value)
        disen_loss = kl_loss_disen.mean() - sum(
            self.config.disen_scales.get(k, 1.0) * v for k, v in likes_disen.items())

        metrics = {f'{name}_loss': -value for name, value in likes.items()}
        ...
        metrics['disen/disen_reward_loss'] = -likes_disen['reward'] / \
            self.config.disen_scales.get('reward', -1.0)
        if self.config.pred_discount:
            metrics['disen/disen_discount_loss'] = -likes_disen['discount'] / \
                self.config.disen_scales.get('discount', -1.0)
        metrics['disen/kl'] = tf.reduce_mean(kl_value_disen)
        metrics['disen/prior_ent'] = self.disen_rssm.get_dist(
            prior_disen).entropy()
        metrics['disen/post_ent'] = self.disen_rssm.get_dist(
            post_disen).entropy()
        metrics.update(
        ...
        return model_loss, disen_loss, last_state, outs, metrics
    ...
    def video_pred(self, data):
        ...
        states_disen, _ = self.disen_rssm.observe(
            embed_disen[:6, :5], disen_action[:6, :5], data['is_first'][:6, :5])
        feats = self.rssm.get_feat(states)
        feats_disen = self.disen_rssm.get_feat(states_disen)
        ...
        prior = self.rssm.imagine(
            data['action'][:6, 5:], init)
        prior_disen = self.disen_rssm.imagine(
            disen_action[:6, 5:], init_disen)
        _feats = self.rssm.get_feat(prior)
        _feats_disen = self.disen_rssm.get_feat(prior_disen)
        ...
        B, T, H, W, C = output_joint.shape
        video_joint = output_joint.transpose((1, 2, 0, 3, 4)).reshape((T, H, B * W, C))
        video_main = output_main.transpose((1, 2, 0, 3, 4)).reshape((T, H, B * W, C))
        video_disen = output_disen.transpose((1, 2, 0, 3, 4)).reshape((T, H, B * W, C))
        video_mask = output_mask.transpose((1, 2, 0, 3, 4)).reshape((T, -1, B * W, 1))
        return video_joint, video_main, video_disen, video_mask
