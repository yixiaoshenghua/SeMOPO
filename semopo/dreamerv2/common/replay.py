...
def load_episodes(directory, capacity=None, minlen=1, keep_temporal_order=False, reverse_temporal=False):
...
class OfflineReplay:
    def __init__(
            self, directory, capacity=0, ongoing=False, minlen=1, maxlen=0,
            prioritize_ends=False, split_val=False, incremental=False, reverse=False, sort_rewards=False):
        self._capacity = capacity
        self._minlen = minlen
        self._maxlen = maxlen
        self._prioritize_ends = prioritize_ends
        ...
        for d in directory:
            ...
            complete_eps = load_episodes(path, capacity, minlen, keep_temporal_order=incremental, reverse_temporal=reverse)
        ...
        self.returns = [ep['reward'].sum() for ep in self.episodes]

        # sort the episodes by the total return
        if sort_rewards:
            episodes_dict = dict(zip(self.returns, self.episodes))
            episodes_dict = dict(sorted(episodes_dict.items(), key=lambda k: k[0], reverse=reverse))
            self.episodes = list(episodes_dict.values())
            self.returns = list(episodes_dict.keys())
    ...
    def _sample_sequence(self, validation):
        if validation:
            ...
            if self.incremental: # random sample the episode following the order of the original collected replay buffer
                idx = self._random.choice(self.curr_idx)
